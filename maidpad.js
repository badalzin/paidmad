...
// Cache de email por nome de funcionário (carregado dinamicamente)
...
function nameToLogin(fullName) {
  ...
  // Direct match
  if (_employeeEmailCache[fullName]) return _employeeEmailCache[fullName];
  // Try first name only
  const firstName = fullName.split(' ')[0];
  for (const [name, email] of Object.entries(_employeeEmailCache)) {
    if (name.split(' ')[0] === firstName) return email;
  }
...
async function exportReviewsToSheets() {
  ...
  if (status) status.textContent = 'Carregando funcionários...';
  await loadEmployeeEmails();
